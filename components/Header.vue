<template>
  <header class="bg-verde text-pastel py-1 shadow-md font-display w-full ">
    <div class="flex gap-1 lg:gap-0 items-center justify-between pl-4 pr-4">
      <!-- Logo -->
      <div
        class="hidden md:flex align-middle items-center gap-1 transition-transform duration-500 ease-in-out hover:scale-105">
        <div class="circulo-externo">
          <div class="circulo-interno">
            <img src="/images/logoSinFondo.png" alt="" class="h-14" />
          </div>
        </div>
        <div class="limon lg:flex text-4xl font-bold font-amatic">Sano Delirio</div>
      </div>
      ...
      <nav class="hidden md:flex space-x-6 align-middle text-center items-center  text-lg font-extrabold limon">
        ...
        <NuxtLink to="/about" class="hover:text-white text-pastel transition-transform duration-300 hover:scale-105">
          Acerca de</NuxtLink>
        <!-- Botón carrito -->
      ...
      <!-- Logo móvil -->
      <div
        class="limon flex gap-4 align-middle items-center justify-center md:hidden text-2xl lg:text-4xl font-bold font-amatic transition-transform duration-300 hover:scale-105">
       <div>
        Sano Delirio
       </div>

        <div class="relative cursor-pointer mr-4 transition-transform duration-300 hover:scale-110 ">
          <button @click="toggleCart" class="text-pastel cursor-pointer flex align-middle items-center justify-center ">
            <Icon name="my-icon:shopping-cart1" class="w-7 h-7 md:w-10 md:h-10" />
            <span v-if="cartItems > 0"
              class="absolute -top-1 -right-2 flex items-center justify-center w-3 h-3 md:w-6 md:h-6 bg-blanco text-red-700 font-extrabold rounded-full text-sm">
              {{ cartItems }}
      ...
      <!-- Botón hamburguesa -->
      <button class="md:hidden text-pastel transition-transform duration-300 hover:scale-110"
